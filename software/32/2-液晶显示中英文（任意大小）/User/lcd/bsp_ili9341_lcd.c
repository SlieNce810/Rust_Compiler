...
static const sFONT *LCD_Currentfonts = &Font8x16;  //英文字体
...
/*
 * VM minimal profile:
 * keep UART/LCD log capability and shrink demo-only zoom buffers to free RAM.
 */
#ifndef VM_LCD_MINIMAL_PROFILE
#define VM_LCD_MINIMAL_PROFILE 1
#endif

#if VM_LCD_MINIMAL_PROFILE
#define ZOOMMAXBUFF 512
#define ZOOMTEMPBUFF 256
#else
#define ZOOMMAXBUFF 16384
#define ZOOMTEMPBUFF 1024
#endif

uint8_t zoomBuff[ZOOMMAXBUFF] = {0};
uint8_t zoomTempBuff[ZOOMTEMPBUFF] = {0};
									...
									uint8_t en_cn)		//0为英文，1为中文	
{
  ...
  unsigned int yrIntFloat_16=(in_heig<<16)/out_heig+1;
	...
	if(in_width * in_heig >= ZOOMTEMPBUFF ) return; 	//限制输入最大，避免临时缓存溢出
...
/**
  ...
  * @retval None
  */
void LCD_SetFont(const sFONT *fonts)
{
...
{
  ILI9341_Clear(0,Line,LCD_X_LENGTH,LCD_GetFont()->Height);	/* 清屏，显示全黑 */
