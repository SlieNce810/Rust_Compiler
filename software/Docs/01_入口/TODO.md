...
- [x] STM32F103 + ESP32S3 双平台点灯与串口日志统一口径
...
- [x] STM32F103：上电后有 `vm_boot/vm_loaded`，LED 可见翻转，异常打印 `vm_run_err`（基于 `Fire_F103VE.uvprojx` 迁移）
- [ ] ESP32S3：同样日志与 LED 行为，输出序列与 STM32 一致（忽略时间戳）

## 下轮（bootloader 协议与动态下载）

- [x] 完成 `Bootloader输入清单.md` 的板级信息补全
- [x] 定义下载包协议（版本、长度、CRC、错误码、兼容策略）
- [x] 在端侧接入串口动态下载并调用 `vm_load_program()`
- [x] 增加校验失败回滚与缓冲区越界保护（当前为“CRC失败回退内置字节码”策略）
- [x] 预留 BLE/WiFi 传输通道（复用下载包协议，当前先落 UART）
